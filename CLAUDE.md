...
- **2026-05-25** — Phase 2c–5 implementation batch. **2c:** [`app/main.py`](backend/app/main.py) FastAPI with `POST /webhook` (secret verification), `POST /internal/process-followups` (cron bearer auth), shared [`app/telegram_app.py`](backend/app/telegram_app.py) PTB factory; `USE_POLLING` gates dev poller vs production webhook; [`infra/supabase/pg_cron_jobs.sql`](infra/supabase/pg_cron_jobs.sql) + [`infra/railway.toml`](infra/railway.toml). **3:** [`base_sq.py`](backend/app/conversation/prompts/base_sq.py), [`app/i18n/`](backend/app/i18n/) detector + strings, `/language`, Albanian inline keyboards. **§9:** [`sanitizer.py`](backend/app/security/sanitizer.py), [`middleware.py`](backend/app/security/middleware.py), [`telegram_auth.py`](backend/app/security/telegram_auth.py), per-user rate limits, `/deletedata`. **4:** Landing how-it-works + trust signals, bilingual Privacy/Terms, language toggle. **5:** [`analytics.py`](backend/app/database/analytics.py) + funnel events. 67 tests pass.
- **2026-05-21** — Phase 2b shipped: Sonnet 4.6 deep validation with real web search. New `app/conversation/validation.py` calls `claude-sonnet-4-6` with Anthropic's server-side `web_search_20250305` tool (capped at 5 searches per call) and asks for a single JSON response: `validation_score` (0-100), `strengths`, `weaknesses`, `competitor_summary` (3-6 sentences naming real competitors found via search, what they do right/wrong, where the gap is), and `market_saturation` (saturated/partial/open). New marker `[VALIDATE]` in `markers.py`; bot calls `engine.run_validation(user)` which writes the score + strengths + weaknesses to the `ideas` row via new `ideas.update_validation`, and records a `validation_feedback` journey event whose `description` field is the competitor summary. `context_builder.build_system_prompt` takes a new `validation_summary` arg; engine loads it via `journey_events.get_latest_validation(idea.id)` so the competitor landscape persists into every future system prompt — Hap can reference real competitors by name from then on. `base_en.py` bumped to 0.6.0 with `[VALIDATE]` documented under Structured signals plus the discipline rule "at most once per idea unless substantially pivoted." `/myidea` now also displays strengths, weaknesses, and the competitor landscape. 56 tests pass.
- **2026-05-21** — Phase 2a shipped: follow-up flow, journey events, and `/homework` + `/progress` commands. New `app/database/journey_events.py` (`record_event`, `list_recent`) records `pivot`, `milestone`, and future `validation_feedback` / `score_change` entries per §5. `homework.py` extended with `list_pending` (drives the new `## PENDING HOMEWORK` block in the system prompt) and `mark_most_recent_sent_pending` (state transition pending → completed/skipped). New markers `[HOMEWORK_DONE]` and `[HOMEWORK_SKIPPED]`: when the LLM sees the user report a task result, the most-recently-sent pending homework flips status and (on completion) a `milestone` event is recorded. `run_idea_extraction` now compares the extracted title to the existing active idea and records a `pivot` journey event when they differ. `context_builder.build_system_prompt` takes optional `pending_homework`; the section instructs the LLM to emit the new markers when appropriate. `base_en.py` bumped to 0.5.0 documenting the new markers + the "don't emit on guesswork" rule. `bot.py` adds `/homework` (lists pending tasks with due descriptions) and `/progress` (timeline of journey events). Phase 2a is testable locally; Phase 2b (Sonnet 4.6 `[VALIDATE]` escalation) and Phase 2c (Railway + pg_cron + webhook handler) follow. 46 tests pass.
